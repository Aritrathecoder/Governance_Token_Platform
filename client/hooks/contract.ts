...
export const CONTRACT_ADDRESS =
  "CDUSAJKA4XEWRO22EW25D4FL5EUIKQA7CSUBD75CSTP5GLSDKGBYRLLI";
...
// ============================================================
// Governance Token Platform — Contract Methods
// ============================================================
 ...
 */
export async function initializeToken(caller: string, admin: string) {
  return callContract("initialize", [toScValAddress(admin)], caller, true);
}
 ...
 */
export async function mintTokens(caller: string, to: string, amount: bigint) {
  return callContract("mint", [toScValAddress(to), toScValI128(amount)], caller, true);
}
...
export async function transferToken(
  ...
  return callContract(
    "transfer",
    [toScValAddress(caller), toScValAddress(to), toScValI128(amount)],
    caller,
    true
  );
}
...
/**
 * Get the token balance of a user (read-only).
 * Calls: balance(user: Address) -> i128
 */
export async function getBalance(user: string, caller?: string) {
